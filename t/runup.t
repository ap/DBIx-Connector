...
$conn->runup(sub {
    my $dbh = shift;
    ok $conn->{_in_run}, '_in_run should be set inside runup()';
    local $dbh->{Active} = 0;
    $conn->runup(sub {
        my $dbha = shift;
        ...
        ok $conn->{_in_run}, '_in_run should be set inside nested runup()';
    });
});
...
$conn->runup(sub {
    is shift, $conn->{_dbh},
        'The txn nested call to runup() should get the deactivated handle';
